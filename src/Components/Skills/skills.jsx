import { motion } from 'framer-motion';
import React, { useRef, useEffect } from 'react'
import LocomotiveScroll from 'locomotive-scroll';

export default function Skills() {
    const scrollRef = useRef(null);
    useEffect(() => {
        const scrollElement = scrollRef.current;
        if (scrollElement) {
            const locomotive = new LocomotiveScroll({
                el: scrollElement,
                smooth: true,
            });
            locomotive.update();
            return () => {
                locomotive.destroy();
            };
        }
    }, []);
    return (
        <>
            <section class="text center items-center body-font" style={{ paddingTop: "100px" }}>
                <div class="container px-5 py-24 mx-auto">
                    <div class="flex flex-col text-center w-full mb-20">
                        <h1 class="text-3xl text-5xl font-medium title-font mb-4 text-white">My Skills</h1>
                        {/* <p class="lg:w-2/3 mx-auto leading-relaxed text-base"></p> */}
                    </div>
                    <SkillTiles />
                </div>
            </section>
        </>
    )
}

function SkillTiles() {
    return (
        <>
            <div class="lg:flex items-center justify-center text-start">
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} class="w-72 items-center text-start justify-center" style={{ padding: "10px" }}>
                    <div class="w-full h-28 max-w-md px-8 py-4 mt-16 rounded-lg shadow-lg bg-black">
                        <div class="flex justify-start md:justify-end">
                            <h1 class="mt-6 text-xl font-semibold text-white">Python</h1>
                            <img class="object-cover w-20 h-20 border-2 border-blue-500 rounded-full ml-auto" alt="" src="https://images.ctfassets.net/mrop88jh71hl/55rrbZfwMaURHZKAUc5oOW/9e5fe805eb03135b82e962e92169ce6d/python-programming-language.png?w=750&h=750&q=100&fm=png" />
                        </div>
                    </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.25 }} class="w-72 items-center text-start justify-center" style={{ padding: "10px" }}>
                    <div class="w-full h-28 max-w-md px-8 py-4 mt-16 rounded-lg shadow-lg bg-black">
                        <div class="flex justify-start md:justify-end">
                            <h1 class="mt-6 text-xl font-semibold text-white">JavaScript</h1>
                            <img class="object-cover w-20 h-20 border-2 border-blue-500 rounded-full ml-auto" alt="" src="data:image/png;base64," />
                        </div>
                    </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.45 }} class="w-72 items-center text-start justify-center" style={{ padding: "10px" }}>
                    <div class="w-full h-28 max-w-md px-8 py-4 mt-16 rounded-lg shadow-lg bg-black">
                        <div class="flex justify-start md:justify-end">
                            <h1 class="mt-6 text-xl font-semibold text-white">Dart</h1>
                            <img class="object-cover w-20 h-20 border-2 border-blue-500 rounded-full ml-auto" alt="" src="https://www.pngitem.com/pimgs/m/480-4800518_file-dart-logo-dart-programming-language-logo-hd.png" />
                        </div>
                    </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.65 }} class="w-72 items-center text-start justify-center" style={{ padding: "10px" }}>
                    <div class="w-full h-28 max-w-md px-8 py-4 mt-16 rounded-lg shadow-lg bg-black">
                        <div class="flex justify-start md:justify-end">
                            <h1 class="mt-6 text-xl font-semibold text-white">Java</h1>
                            <img class="object-cover w-20 h-20 border-2 border-blue-500 rounded-full ml-auto" alt="" src="https://1000logos.net/wp-content/uploads/2020/09/Java-Emblem.jpg" />
                        </div>
                    </div>
                </motion.div>
            </div>
            <div class="lg:flex items-center justify-center text-start">
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} class="w-72 items-center text-start justify-center" style={{ padding: "10px" }}>
                    <div class="w-full h-28 max-w-md px-8 py-4 mt-16 rounded-lg shadow-lg bg-black">
                        <div class="flex justify-start md:justify-end">
                            <h1 class="mt-6 text-xl font-semibold text-white">Solidity</h1>
                            <img class="object-cover w-20 h-20 border-2 border-blue-500 rounded-full ml-auto" alt="" src="https://pbs.twimg.com/media/E0KH3AOX0AQPjKa.png" />
                        </div>
                    </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.25 }} class="w-72 items-center text-start justify-center" style={{ padding: "10px" }}>
                    <div class="w-full h-28 max-w-md px-8 py-4 mt-16 rounded-lg shadow-lg bg-black">
                        <div class="flex justify-start md:justify-end">
                            <h1 class="mt-6 text-xl font-semibold text-white">C</h1>
                            <img class="object-cover w-20 h-20 border-2 border-blue-500 rounded-full ml-auto" alt="" src="https://i.pinimg.com/originals/6e/46/e7/6e46e7dbe2bb73dacc055e5dbd85c3ad.png" />
                        </div>
                    </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.45 }} class="w-72 items-center text-start justify-center" style={{ padding: "10px" }}>
                    <div class="w-full h-28 max-w-md px-8 py-4 mt-16 rounded-lg shadow-lg bg-black">
                        <div class="flex justify-start md:justify-end">
                            <h1 class="mt-6 text-xl font-semibold text-white">HTML</h1>
                            <img class="object-cover w-20 h-20 border-2 border-blue-500 rounded-full ml-auto" alt="" src="data:image/png;base64," />
                        </div>
                    </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.65 }} class="w-72 items-center text-start justify-center" style={{ padding: "10px" }}>
                    <div class="w-full h-28 max-w-md px-8 py-4 mt-16 rounded-lg shadow-lg bg-black">
                        <div class="flex justify-start md:justify-end">
                            <h1 class="mt-6 text-xl font-semibold text-white">CSS</h1>
                            <img class="object-cover w-20 h-20 border-2 border-blue-500 rounded-full ml-auto" alt="" src="https://cdn.pixabay.com/photo/2017/08/05/11/16/logo-2582747_1280.png" />
                        </div>
                    </div>
                </motion.div>
            </div>
            <div class="lg:flex items-center justify-center text-start">
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.85 }} class="w-72 items-center text-start justify-center" style={{ padding: "10px" }}>
                    <div class="w-full h-28 max-w-md px-8 py-4 mt-16 rounded-lg shadow-lg bg-black">
                        <div class="flex justify-start md:justify-end">
                            <h1 class="mt-6 text-xl font-semibold text-white">React.js</h1>
                            <img class="object-cover w-20 h-20 border-2 border-blue-500 rounded-full ml-auto" alt="" src="data:image/png;base64," />
                        </div>
                    </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.25 }} class="w-72 items-center text-start justify-center" style={{ padding: "10px" }}>
                    <div class="w-full h-28 max-w-md px-8 py-4 mt-16 rounded-lg shadow-lg bg-black">
                        <div class="flex justify-start md:justify-end">
                            <h1 class="mt-6 text-xl font-semibold text-white">AngularJS</h1>
                            <img class="object-cover w-20 h-20 border-2 border-blue-500 rounded-full ml-auto" alt="" src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/cf/Angular_full_color_logo.svg/2048px-Angular_full_color_logo.svg.png" />
                        </div>
                    </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.45 }} class="w-72 items-center text-start justify-center" style={{ padding: "10px" }}>
                    <div class="w-full h-28 max-w-md px-8 py-4 mt-16 rounded-lg shadow-lg bg-black">
                        <div class="flex justify-start md:justify-end">
                            <h1 class="mt-6 text-xl font-semibold text-white">Flutter</h1>
                            <img class="object-cover w-20 h-20 border-2 border-blue-500 rounded-full ml-auto" alt="" src="https://logowik.com/content/uploads/images/flutter5786.jpg" />
                        </div>
                    </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.65 }} class="w-72 items-center text-start justify-center" style={{ padding: "10px" }}>
                    <div class="w-full h-28 max-w-md px-8 py-4 mt-16 rounded-lg shadow-lg bg-black">
                        <div class="flex justify-start md:justify-end">
                            <h1 class="mt-6 text-xl font-semibold text-white">Django</h1>
                            <img class="object-cover w-20 h-20 border-2 border-blue-500 rounded-full ml-auto" alt="" src="https://copyassignment.com/wp-content/uploads/2022/06/Django-logo-474x360-1.jpg" />
                        </div>
                    </div>
                </motion.div>
            </div>
            <div class="lg:flex items-center justify-center text-start">
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} class="w-72 items-center text-start justify-center" style={{ padding: "10px" }}>
                    <div class="w-full h-28 max-w-md px-8 py-4 mt-16 rounded-lg shadow-lg bg-black">
                        <div class="flex justify-start md:justify-end">
                            <h1 class="mt-6 text-xl font-semibold text-white">AWS</h1>
                            <img class="object-cover w-20 h-20 border-2 border-blue-500 rounded-full ml-auto" alt="" src="https://pbs.twimg.com/profile_images/1641476962362302464/K8lb6OtN_400x400.jpg" />
                        </div>
                    </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.25 }} class="w-72 items-center text-start justify-center" style={{ padding: "10px" }}>
                    <div class="w-full h-28 max-w-md px-8 py-4 mt-16 rounded-lg shadow-lg bg-black">
                        <div class="flex justify-start md:justify-end">
                            <h1 class="mt-6 text-xl font-semibold text-white">Git</h1>
                            <img class="object-cover w-20 h-20 border-2 border-blue-500 rounded-full ml-auto" alt="" src="https://upload.wikimedia.org/wikipedia/commons/thumb/3/3f/Git_icon.svg/1200px-Git_icon.svg.png" />
                        </div>
                    </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.25 }} class="w-72 items-center text-start justify-center" style={{ padding: "10px" }}>
                    <div class="w-full h-28 max-w-md px-8 py-4 mt-16 rounded-lg shadow-lg bg-black">
                        <div class="flex justify-start md:justify-end">
                            <h1 class="mt-6 text-xl font-semibold text-white">Firebase</h1>
                            <img class="object-cover w-20 h-20 border-2 border-blue-500 rounded-full ml-auto" alt="" src="https://logowik.com/content/uploads/images/firebase.jpg" />
                        </div>
                    </div>
                </motion.div>
            </div>
            <div class="lg:flex items-center justify-center text-start">
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.25 }} class="w-72 items-center text-start justify-center" style={{ padding: "10px" }}>
                    <div class="w-full h-28 max-w-md px-8 py-4 mt-16 rounded-lg shadow-lg bg-black">
                        <div class="flex justify-start md:justify-end">
                            <h1 class="mt-6 text-xl font-semibold text-white">MySQL</h1>
                            <img class="object-cover w-20 h-20 border-2 border-blue-500 rounded-full ml-auto" alt="" src="https://logowik.com/content/uploads/images/mysql8604.logowik.com.webp" />
                        </div>
                    </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} class="w-72 items-center text-start justify-center" style={{ padding: "10px" }}>
                    <div class="w-full h-28 max-w-md px-8 py-4 mt-16 rounded-lg shadow-lg bg-black">
                        <div class="flex justify-start md:justify-end">
                            <h1 class="mt-6 text-xl font-semibold text-white">Visual Studio<br /> Code</h1>
                            <img class="object-cover w-20 h-20 border-2 border-blue-500 rounded-full ml-auto" alt="" src="https://logowik.com/content/uploads/images/visual-studio-code7642.jpg" />
                        </div>
                    </div>
                </motion.div>
            </div>
        </>
    )
}